...
actual class MediaPlayerNativePlayer : NativePlayer {
    ...
    fun preparePlayer(
        ...
        player?.addListener(object : MediaPlayerController.Listener {
            ...
            override fun onPlaybackStateChanged(p0: MediaPlayerController, p1: Int, p2: Int) {
                if (p1 == STOPPED && p2 == PAUSED) { p0.play() }
            }
    ...
    override fun set(queue: List<SongEntry>) {
        ...
        val catalog = CatalogPlaybackQueueItemProvider
            .Builder()
            .items(MediaItemType.SONG, first.id)
            .build()

        player?.prepare(catalog)

        mutable.forEach { entry ->
            val item = CatalogPlaybackQueueItemProvider
                .Builder()
                .items(MediaItemType.SONG, entry.id)
                .build()
            player?.addQueueItems(item, PlaybackQueueInsertionType.INSERTION_TYPE_AT_END)
        }
    }
